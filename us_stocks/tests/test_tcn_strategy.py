...
from us_invest_ai.tcn_strategy import (
    TCNModelConfig,
    fit_tcn_model,
    generate_tcn_target_weights,
    predict_tcn_model,
)


class TCNStrategyTests(unittest.TestCase):
    def test_fit_tcn_model_learns_sequence_relationship(self) -> None:
        ...
        train_targets = np.array([4.0, 6.0, 8.0, 10.0, 12.0], dtype=np.float32)
        ...
        validation_targets = np.array([7.0, 9.0], dtype=np.float32)
        ...
        predicted = predict_tcn_model(
            model,
            np.array([[[3.5], [4.5], [5.5]]], dtype=np.float32),
        )

        self.assertAlmostEqual(float(predicted[0]), 11.0, delta=1.0)
        self.assertGreaterEqual(model.validation_mse, 0.0)

    def test_generate_tcn_target_weights_prefers_stronger_trend_asset(self) -> None:
        ...
        weights, history = generate_tcn_target_weights(
            features=features,
