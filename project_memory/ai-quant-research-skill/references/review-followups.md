...
- Added a signal-hardening degradation report that reads the promoted last-year and repeated-window summaries, emits a robustness leaderboard plus JSON and Markdown guidance artifacts, and now runs inside `run_us_report_stack.ps1`.
...
- Turn the new signal-hardening degradation thresholds into explicit pass/fail promotion gates instead of advisory artifacts only.
